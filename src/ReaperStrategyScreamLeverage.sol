...
contract ReaperStrategyScreamLeverage is ReaperBaseStrategyv4 {
  ...
  function setWftmToWantRoute(address[] calldata _newWftmToWantRoute) external {
    ...
    require(
      _newWftmToWantRoute[_newWftmToWantRoute.length - 1] == want,
      "bad route"
    );
  ...
  function _leverUpStep(uint256 _withdrawAmount) internal returns (uint256) {
    ...
    if (_withdrawAmount > 10) {
      ...
      IERC20Upgradeable(want).safeIncreaseAllowance(address(cWant), mintAmount);
   ...
   */
  function _calculateLTVAfterWithdraw(uint256 _withdrawAmount)
    internal
    returns (uint256 ltv)
  {
    uint256 supplied = cWant.balanceOfUnderlying(address(this));
   ...
   */
  function _leverDownStep(uint256 maxDeleverage)
    internal
    returns (uint256 deleveragedAmount)
  {
